// ===========================
// PROFILE PAGE LOGIC — V2
// ===========================

// --- 1. Get the creative ---
const params = new URLSearchParams(window.location.search);
const creativeId = params.get('id') || 999; // default to Modisa during dev
const creative = getCreativeById(creativeId);

if (!creative) {
  document.body.innerHTML = '<p style="color:white;padding:2rem;">Creative not found.</p>';
  throw new Error('No creative with that ID');
}

const cat = CATEGORIES[creative.category];
const isMe = creative.isMe === true;

// --- 2. Set fade color from category ---
document.documentElement.style.setProperty('--fade-color', cat.colorHex);

// --- 3. Populate text content ---
document.getElementById('creativeName').textContent = creative.name;
...
// --- 4. Top-right icon: pencil if it's me, dots otherwise ---
...
if (creative.photos && creative.photos.length > 0) {
  photoStack.innerHTML = creative.photos.map((url, i) => `
    <div class="photo-slide ${i === 0 ? 'is-visible' : ''}" data-index="${i}">
      <div class="photo-slide__img" style="background-image: url('${url}');"></div>
    </div>
  `).join('');

  // Hide scroll hint if there's only one photo
  if (creative.photos.length <= 1) {
...
// --- 6. Track which slide is visible (for ken-burns zoom + hide hint at bottom) ---
const mediaEl = document.getElementById('media');

// IntersectionObserver = the modern, performant way to detect "is this on screen?"
// Way better than listening to every scroll event.
const observer = new IntersectionObserver((entries) => {
  ...
  root: mediaEl,
  threshold: 0.6 // slide counts as "visible" once 60% of it is on screen
});

document.querySelectorAll('.photo-slide').forEach(slide => observer.observe(slide));

// Hide scroll hint once user has scrolled past the first slide
mediaEl.addEventListener('scroll', () => {
  if (mediaEl.scrollTop > 50) {
    scrollHint.classList.add('is-hidden');
  } else if (creative.photos && creative.photos.length > 1) {
    scrollHint.classList.remove('is-hidden');
  }
});

// --- 7. Like button (inactive, animated) ---
const likeBtn = document.getElementById('likeBtn');
likeBtn.addEventListener('click', () => {
  likeBtn.classList.remove('is-playing');
  void likeBtn.offsetWidth; // force reflow → re-trigger animation
  likeBtn.classList.add('is-playing');
  setTimeout(() => likeBtn.classList.remove('is-playing'), 800);
});

// --- 8. Double-click photo to enter focus mode (UI hides for clearer view) ---
...
// --- 11. Fade color picker — owner only ---
// Hard-removed from DOM (not just hidden) when not the owner.
...
if (isMe) {
  ...
  // Not the owner — fully remove the button so it can't be enabled via inspector
  fadeColorBtn.remove();
}

// --- 12. Top-right edit / back ---
topRightBtn.addEventListener('click', () => {
  if (isMe) {
    console.log('[Connekkt] Edit profile flow — coming next round.');
  } else {
    console.log('[Connekkt] More options menu — coming later.');
  }
});
...
// SVGs reused for each contact method
const contactIcons = {
...
// Build the right link for each contact type
function buildContactRow(type, value) {
  const icon = `<div class="contact-row__icon">${contactIcons[type]}</div>`;
  ...
  // Notes are not clickable — they're just info
  if (type === 'note') {
    return `<div class="contact-row contact-row--note">${inner}</div>`;
  }

  // Build the right deep link per type
  // mailto: opens email app, tel: triggers call, wa.me/ opens WhatsApp
  let href = '#';
...
// Populate modal contents based on what this creative chose to share
function renderContactModal() {
  ...
  if (rows.length === 0) {
    // Empty state — creative shared nothing direct
    contactSubtitle.textContent = `${creative.name} prefers to keep things on the platform.`;
    contactList.innerHTML = `
      <div class="contact-empty">
        No direct contact info shared yet. Once Connekkt messaging launches, you'll be able to reach out here.
      </div>
    `;
  } else {
...
// ESC key closes the modal too — small accessibility win
